...
const { v4: uuidv4 } = require('uuid'); // ← move to top
...
router.post("/register", async (req, res) => {
  try {
    ...
    if (!email.endsWith("@gmail.com")) return res.status(400).json({ msg: "Only Gmail allowed" });
    if (password.length < 6) return res.status(400).json({ msg: "Password must be at least 6 characters" });
    const userExists = await User.findOne({ email });
    if (userExists) return res.status(400).json({ msg: "Email already used" });
    const hashedPassword = await bcrypt.hash(password, 10);
    const user = new User({ nom, prenom, telephone, email, password: hashedPassword, adresse });
    await user.save();
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!email || !password) return res.status(400).json({ msg: "Email et password obligatoires" });
...
router.post("/choose-role", async (req, res) => {
  try {
    ...
    if (!["client", "fournisseur", "gerant"].includes(role)) {
